  ...
  return (
    <div className="space-y-2">
      {criteria.map((criterion) => {
        ...
        return (
          <div key={criterion.id} className="rounded-lg border p-2.5">
            <div className="mb-1.5">
              <div className="text-sm font-medium">{criterion.name}</div>
              {criterion.description && (
                <div className="line-clamp-1 text-xs text-muted-foreground">{criterion.description}</div>
              )}
            </div>
              ...
              {([0, 1, 2, 3] as const).map((level) => {
                ...
                return (
                  <button
                    ...
                    className={cn(
                      "min-h-[52px] rounded-lg border p-2 text-left text-xs transition-colors",
                      isSelected
